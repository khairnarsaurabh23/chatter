...
exports.addGroup = (req, res, next) => {
  ...
  if (body !== undefined && token !== undefined) {
    jwt.verify(token, SECRET_KEY, async function (err, decryptToken) {
      ...
      console.log("decrypted token here",decryptToken)
      ...
      try {
        ...
        try {
          ...
          //find the number of users in a group
...
exports.getUserGroupInformation = (req, res, next) => {
  ...
  if (body !== undefined && token !== undefined) {
    jwt.verify(token, SECRET_KEY, async function (err, decryptToken) {
      ...
      console.log("decrypt token", decryptToken)
  ...
  if (token !== undefined && group) {
    jwt.verify(token, SECRET_KEY, async function (err, decryptToken) {
      ...
      try {
        ...
        console.log("messages here", typeof messageObj)
